...
global $DBConn;
if ($DBConn) {
	$r = $DBConn->query("SELECT c.name as collaborator_name, b.name as bu_name, s.label as status_name, bc.* FROM my_collaborator c, my_bu b, my_status s, my_bu_collaborator bc WHERE bc.collaborator = c.id AND bc.bu = b.id AND bc.status = s.id;");
	if ($r) {
		foreach ($r as $row) {
			$data[] = $row;
		}
