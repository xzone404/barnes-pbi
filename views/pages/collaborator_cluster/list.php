...
global $DBConn;
if ($DBConn) {
	$r = $DBConn->query("SELECT CONCAT(c.firstname, ' ', c.lastname) as collaborator_name, g.name as cluster_name, s.label as status_name, bc.* FROM my_collaborator c, my_cluster g, my_status_type s, my_cluster_collaborator bc WHERE bc.collaborator = c.id AND bc.cluster = g.id AND bc.status = s.id;");
	if ($r) {
		foreach ($r as $row) {
			$data[] = $row;
		}
